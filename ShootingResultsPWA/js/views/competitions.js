...
export function renderCompetitions(root) {
    ...
    sorted.forEach((c) => {
      ...
      const item = el("div", { class: `list-item${isActive ? " active" : ""}` }, [
        el("div", {
          ...
          el("div", { class: "title" }, [
            c.name || t("Bez názvu"),
            isActive ? el("span", { class: "pill", style: "margin-left:8px", text: t("aktivní") }) : null,
          ]),
          el("div", { class: "sub", text: `${t("Naposledy upraveno:")} ${fmtDate(c.updatedAt)}` }),
        ]),
        el("div", { class: "btn-row" }, [
          el("button", {
            class: "btn-ghost btn-sm", text: "✏️",
            onclick: () => {
              const name = prompt(t("Nový název soutěže:"), c.name || "");
          ...
          el("button", {
            ...
            onclick: () => {
              ...
              if (confirm(`${t("Opravdu smazat soutěž")} "${c.name}"? ${t("Tohle nejde vrátit zpět.")}`)) {
  ...
  const addBtn = el("button", {
    ...
    text: t("+ Nová soutěž"),
    onclick: () => {
      const name = prompt(t("Název nové soutěže:"), "");
      if (name === null) return;
  ...
  view.append(
    el("p", { style: "color:var(--text-muted);font-size:13px", text: t("Aplikace umí držet víc rozpracovaných/dokončených soutěží najednou. Klikni na soutěž pro přepnutí.") }),
    listWrap,
    addBtn,
  );
